...
class CommentManager(models.Manager):
    ...
    def _filter_parents(qs, parent=None):
        return qs.filter(parent=parent, is_active=True).order_by(*settings.COMMENT_ORDER_BY)
    ...
    def all_comments_by_object(self, obj, include_flagged=False):
        ...
        return self.all_exclude_flagged().filter(content_type=content_type, object_id=obj.id, is_active=True)
